...
import com.thinkgem.jeesite.modules.pdd.bean.*;
...
public class ChartService {
    ...
    public DataBean getLineChartData4() {
        String[] categories = new String[] {"10-11", "10-12", "10-13", "10-14", "10-15"};

        List<SeriesBean> seriesBeans = new ArrayList<SeriesBean>();
        seriesBeans.add(new SeriesBean("已揽件", "#76FF32", new double[] {3, 2, 1, 3, 4}));
        seriesBeans.add(new SeriesBean("在中", "#a209f4", new double[] {3, 3, 1, 3, 4}));
        seriesBeans.add(new SeriesBean("已到", "#ff33ff", new double[] {3, 2, 1, 5, 4}));
        seriesBeans.add(new SeriesBean("异常", "#ff33ff", new double[] {3, 2, 0, 3, 4}));
        seriesBeans.add(new SeriesBean("失败", "#ff33ff", new double[] {3, 2, 8, 3, 4}));



        return new DataBean("container", "混合图表", "数量", "日期", Arrays.asList(categories), seriesBeans);
    }
